...
#
# Marker format: "<state>:<epoch>" where state ∈ {implementer, reviewer} and
...
usage() {
  cat >&2 <<'EOF'
Usage: spec-reviewer-marker.sh <write implementer|write reviewer|clear>

  write implementer  — record an implementer dispatch
  write reviewer     — record a spec-reviewer PASS
  ...
  write)
    case "$arg" in
      implementer|reviewer)
        ...
        ;;
      *)
        echo "error: write requires 'implementer' or 'reviewer'" >&2
        usage
        exit 2
        ;;
    esac
